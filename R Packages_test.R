...
################################################################################
#1、2、3、4步单次测试只运行其中一个
#1、测试STRING数据库处理后的但数据运行RWR，观察预测结果
...
#2、测试STRING、PRIN、RicePPInet三数据库数据整理后取并集运行RWR，观察预测结果
...
#三四步co_expression_table数据是基于deal_data.R更改
#3、测试第二步的融合PPI数据和基于差异表达基因筛选后的表达矩阵建立的共表达网络运行RWR-M
...
#4、测试第二步的融合PPI数据和基于MIC值筛选后的表达矩阵建立的共表达网络运行RWR-M
PPI_table <- read.csv("DATA/PPI_2021_9_4.txt",header = FALSE,sep = " ")
co_expression_table <- read.csv("DATA/expression_2021_9_4.txt",header = FALSE,sep = " ")
co_expression_table <- co_expression_table[,1:2]
...
#读入种子节点
#1是PPI数据库取了阈值的，2是[[I数据库没取阈值]
Seed_Nodes <- read.csv("DATA/1-Test_Seeds_brought_P0.3.txt",sep = " ",header = FALSE)
Seed_Nodes <- read.csv("DATA/2-Test_Seeds_brought_MIC0.txt",sep = " ",header = FALSE)
...
write.table(LOOCV_Result,file = 'DATA/3.27-nai_Drought_LOOCV_Result.txt',sep = " ",row.names =FALSE, col.names =FALSE, quote =FALSE)
LOOCV_Result <- read.csv('DATA/3.27-nai_Drought_LOOCV_Result.txt',sep = " ",header = FALSE)
